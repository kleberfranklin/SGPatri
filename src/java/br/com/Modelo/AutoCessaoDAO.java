...
//METODO utilizado na Classe (AutoCessaoVencida) terceiro
    public int qtdAutoCessaoVencidaTerceiro(){
          String sql = ("SELECT COUNT(*) as total FROM autocessaocompletoterceiro "
                    + "WHERE dt_vencimento <= ? "
                    + "and nr_vigor = 1 and nr_encerrado = 0 "
                    + "ORDER BY dt_vencimento DESC");
                try {
...
//METODO utilizado na Classe (AutoCessaolistaPagFiltro)Terceiro
    public int qtdAutoCessaoPesquisaTerceiro(String qTpcessao, String qAC, String qProcesso, String qCessionario, 
            String qEndereco,String qCroqui ,String qVigor){
            String sql = ("SELECT COUNT(*) as total FROM autocessaocompletoterceiro WHERE nm_cessao like ? and nm_ac like ? and nm_processo like ? "
                  + "and nm_cessionario like ? and nm_endereco like ? and nm_croqui like ? and nr_Vigor like ?");
           try {
               PreparedStatement stmt = connection.prepareStatement(sql);
                   stmt.setString(1, '%'+qTpcessao+'%');
...
//METODO utilizado na Classe (AutoCessaoPrazo) terceiro
    public int qtdAutoCessaoPrazoTerceiro( int mes, int ano){
          String sql = ("SELECT COUNT(*) as total FROM autocessaocompletoterceiro "
                        + "WHERE dt_vencimento >= ? and month(dt_vencimento) = ? and year(dt_vencimento) = ? ");
           try {
               PreparedStatement stmt = connection.prepareStatement(sql);
