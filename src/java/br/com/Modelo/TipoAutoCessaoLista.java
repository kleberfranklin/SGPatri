 ...
 * @author d732229
 */
public class TipoAutoCessaoLista implements Logica {
    @Override
...
//Carregando a quantidade de registro para calculdo da quantidade de paginas        
        qtdRegistro = tpCessaoDAO.qdTipoCessao(q);
...
//Logica da paginação            
    ...
    //Populando o objeto lista 
        List<TipoAutoCessao> listTpCessao = new TipoAutoCessaoDAO().listTpCessao(qtdLinha, offset, q);
        req.setAttribute("listTpCessao", listTpCessao);
    
        return "TipoAutoCessaoLista.jsp?pg="+pg+"&pi="+pi+"&pf="+pf+"&qtdPg="+qtdPg+"&totalRes="+qtdRegistro+"&q="+q;
